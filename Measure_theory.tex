...
\end{exercise}
\begin{proof}
Since each $f_n\in L^1(\mu)$ we have that \[\int f_n^+d\mu,\int f_n^-d\mu<\infty\forall n\in\mathbb{N}.\] By the monotone convergence theorem we have \[\int f^+d\mu=\lim_{n\to\infty}\int f_n^+d\mu<\infty.\] Furthermore, since $|f_n^-(x)|\leq f_1^-(x)\forall n\in\mathbb{N}$, the dominated convergence theorem implies that \[\int f^-d\mu=\lim_{n\to\infty}\int f_n^-d\mu <\infty.\] Hence \[\int|f|d\mu=\int f^++f^-d\mu<\infty\] so $f\in L^1(\mu)$. Furthermore, since \[\sup_n\int f_nd\mu<\infty\] we know that \[\lim_{n\to\infty}\int f_nd\mu\] exists and is finite, since increasing sequences which are bounded converge to a finite real number. By Fatou's lemma we have \[\int fd\mu=\int\liminf_{n\to\infty}f_nd\mu\leq\liminf_{n\to\infty}\int f_nd\mu=\lim_{n\to\infty}\int f_nd\mu.\] Furthermore, since $f_n\uparrow f$ pointwise, we have that \[\int f_nd\mu\leq\int fd\mu\forall n\] and hence \[\lim_{n\to\infty}\int f_nd\mu\leq\int fd\mu.\] Combining these two inequalities then gives \[\lim_{n\to\infty}\int f_nd\mu=\int fd\mu.\]
\end{proof}
