...
\begin{enumerate}
    ...
    \item[(b)] There is a unique measure $\pi^*$ on $\sigma(\mathcal{S})$ which agrees with $\pi$ on $\mathcal{S}$. Measures have countable additivity so $\pi^*(\bigcup_{n=1}^\infty A_n)=\sum_{n=1}^\infty\pi^*(A_n)$. $\bigcup_{n=1}^\infty A_n,A_1,A_2,...\in\mathcal{S}$ so $\pi(\bigcup_{n=1}^\infty A_n)=\sum_{n=1}^\infty\pi(A_n)$.
\end{enumerate}
\end{proof}
\begin{exercise}
...
\item[(b)] Taking \((\Omega, \mathcal{F}, P)\) to be a probability space, and random variables (i.e., measurable functions) \( Y_1, Y_2, \dots : \Omega \to \mathbb{R} \) show that for any constant \(\mu \in \mathbb{R}\) the set:
    ...
    \]
    is in \(\mathcal{F}\). Deduce that expressions like \( P[\lim\limits_{n \to \infty} \frac{1}{n} \sum_{i=1}^{n} Y_i = \mu] \) are meaningful.
\end{enumerate}
\end{exercise} 
\begin{exercise}
